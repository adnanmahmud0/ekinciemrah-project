          ...
          {PRODUCTS.map((product) => (
            <Link key={product.id} href={`/service/${product.id}`} className="block group">
              <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 hover:shadow-xl transition-shadow">
                {/* Product Image */}
                <div className="relative h-40 md:h-64 w-full bg-gray-50 overflow-hidden">
                  <Image
                    src={product.image}
                    alt={product.name}
                    fill
                ...
                <div className="p-3 md:p-6">
                  <div className="flex flex-col md:flex-row items-start justify-between mb-2 md:mb-3 gap-1">
                    <h4 className="text-sm md:text-xl font-bold text-[#0D1E32] line-clamp-1">{product.name}</h4>
                    <div className="flex items-center gap-1 text-xs md:text-sm">
                      <Star className="w-3 h-3 md:w-4 md:h-4 fill-green-500 text-green-500" />

...
export default function CheckoutCartItem({ item, onUpdateQuantity, onRemove }: CheckoutCartItemProps) {
    ...
    return (
        <div className="bg-[#E6F4F1] rounded-xl p-4 flex gap-4 items-center">
            ...
            <div className="flex-1 min-w-0">
                <h3 className="font-bold text-[#0D1E32] mb-1 truncate">{item.name}</h3>
                <p className="text-sm text-gray-600 mb-2 line-clamp-1">{item.description}</p>
                <div className="flex items-center justify-between">
                    <div className="text-sm">
                        <span className="font-bold text-[#146041]">$ {item.price.toFixed(2)}</span>
